...
public class BiomeFill extends Biome {


	public BiomeFill(BiomeProperties properties) {
		super(properties);
	}
		...
		for (int j1 = 255; j1 >= 0; --j1) {
			if (j1 <= rand.nextInt(5)) {
				...
				if (iblockstate2.getMaterial() == Material.AIR) {
					j = -1;
				} else if (iblockstate2.getBlock() == Blocks.STONE || iblockstate2.getBlock() == fillerBlock.getBlock()) {
					if (j == -1) {
						if (k <= 0) {
							iblockstate = AIR;
