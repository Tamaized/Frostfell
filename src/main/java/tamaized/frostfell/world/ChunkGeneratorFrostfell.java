...
public class ChunkGeneratorFrostfell implements IChunkGenerator {
	...
	private MapGenBase caveGenerator = new MapGenCaves() {
		...
		protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop, IBlockState state, IBlockState up) {
			...
			if (this.canReplaceBlock(state, up) || state.getBlock() == top.getBlock() || state.getBlock() == filler.getBlock()) {
				dig(data, x, y, z, foundTop, top, filler);
			}
		}
	};
	private MapGenBase ravineGenerator = new MapGenRavine() {
		...
		protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
			...
			if (state == FILLER_BLOCK || state.getBlock() == top.getBlock() || state.getBlock() == filler.getBlock()) {
				dig(data, x, y, z, foundTop, top, filler);
			}
		}
	...
	public ChunkGeneratorFrostfell(World worldIn, long seed) {
		settings = new ChunkGeneratorSettings.Factory().build();
		{
	...
	private static void dig(ChunkPrimer data, int x, int y, int z, boolean foundTop, IBlockState top, IBlockState filler) {
		if (y - 1 < 10) {
		...
		for (int rx = 0; rx < 16; rx++)
			for (int rz = 0; rz < 16; rz++)
				for (int ry = 0; ry < 256; ry++)
					if (chunkprimer.getBlockState(rx, ry, rz) == Blocks.STONE.getDefaultState())
						chunkprimer.setBlockState(rx, ry, rz, FILLER_BLOCK);
